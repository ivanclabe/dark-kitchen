-- Corrección de dk_inventory_signals: un insumo NO perecedero salía con
-- perishable_risk = true, porque greatest(null, 0) devuelve 0 en Postgres y
-- la merma proyectada se calculaba sin fecha de vencimiento. Ahora el riesgo
-- solo aplica cuando hay días restantes estimados (perecedero con vida útil).
...
as $$
  ...
  select
    ...
    -- (days_left null = no perecedero o sin fecha: nunca en riesgo. Ojo: greatest(null, 0) = 0 en Postgres.)
