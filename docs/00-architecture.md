...
- `dk_users` (perfil ligado a `auth.users.id`) guarda el `role`. Una función `SECURITY DEFINER` `dk_current_role()` la consulta sin recursión de RLS, y se usa dentro de las políticas: `USING (dk_current_role() IN ('ADMIN','MANAGER'))`.
